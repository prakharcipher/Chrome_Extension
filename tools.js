...
jQuery.fn.extend({
  getSelection: function() {
    ...
    input.onmousedown = function() {
      // for IE because it loses caret position when focus changed
  ...
  replaceSelection: function(inputStr) {
    ...
    if (document.selection && typeof input.selectionStart != 'number') {
      ...
      typeof input.selectionStart == 'number' && // MOZILLA support
      input.selectionStart != input.selectionEnd
    ) {
      start = input.selectionStart;
      end = input.selectionEnd;
      input.value =
        input.value.substr(0, start) + inputStr + input.value.substr(end);
